...
def run_test_integers():
    ...
    # Test 2:
    expected = [1, -7, 18, 4, 12]
    answer = integers([(3.1, 1, 2.4),
                       (1.0, ' '),
                       [-7, 18, 'string', 4],
                       'help',
                       [],
...
def run_test_big_letters():
    ...
    expected = 'PPPER'
    answer = big_letters([(3, 1, 4),  # not a string
                          'Please tell me',  # OTSSS
                          (-15, 'aRe you alrIght maN?', 'elephanT'),  # not a
                          # string
                          [],  # not a string
                          ['ERrrrr'],  # not a string
                          'ooPs',  #
                          ['whAt is life?'],  # not a string
                          '1 ooPs !',  # OOPS
                          'E',  # A
                          '$$&*#%&&',  # PS
                          ('l'),  # B
        ...
        if type((sequence_of_sequences[k])) is str:
            for j in range(len(sequence_of_sequences[k])):
                if ((sequence_of_sequences[k])[j]).isupper() == True:
                    list = list.__add__((sequence_of_sequences[k])[j])
    return list
